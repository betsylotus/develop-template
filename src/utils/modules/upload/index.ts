...
const getCategoryTypes = (categories: string[]): string[] => {
	return categories
		.map((category) => FILE_CATEGORIES[category]?.types || [])
		.flat();
};
	...
	return new Promise((resolve) => {
		...
		reader.onloadend = (e: ProgressEvent<FileReader>) => {
			...
			resolve({
				...
				errorType: isValid
					? undefined
					: FileValidationError.MAGIC_NUMBER_MISMATCH
			});
		};
...
export const validateFile = async (
	...
	try {
		...
		const finalAllowedTypes = [
			...allowedTypes,
			...getCategoryTypes(allowedCategories)
		];
		...
		// 魔数验证
		if (strictMode) {
			const magicNumberResult = await validateFileType(
				file,
				finalAllowedTypes,
				true
			);
